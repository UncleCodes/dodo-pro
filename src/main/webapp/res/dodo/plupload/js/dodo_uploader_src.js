;(function ($,w) {
    ...
    var uploaderTpl = [
        ...
        '<a style="color:#1E9FFF" id="${buttonId}" href="#"><b class="icon-add"></b>${chooseFileLabel}</a>',
        '</div>',
    ...
    var fileItemTpl = [
        ...
        '</span>',
        '${fileHiddenPostHtml}',
        '<a href="#" style="color:#1E9FFF" class="removeFile">${removeItemLabel}</a>',
        '${fileAttrLocation}',
        '</li>'
    ].join('');
    
    var extAttrTpl='<div class="layui-form-mid layui-word-aux">${attrKeyName}:</div><input type="text" class="attrVal layui-input" value="" name="${fileId}-key-${attrKey}" style="width:50%;"/>&nbsp;&nbsp;';
    var hiddenItemTpl = [
        ...
        uploader.addDisplayFile = function (files) {
            ...
            if ($.dodo.isArray(files)) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if(isVideo){
                    	tpl = tpl.replace('<a href="#" style="color:#1E9FFF" class="removeFile">'+removeItemLabel+'</a>', '<a href="#" style="color:#1E9FFF" class="removeFile">'+removeItemLabel+'</a>'+getVideoItemTpl(file));
                    }
                    if (file.canDelete === false) {
                        tpl = tpl.replace('<a href="#" style="color:#1E9FFF" class="removeFile">'+removeItemLabel+'</a>', '');
                    }
        ...
        uploader.disableAddFile = function (bool) {
            ...
            if (bool) {
                ...
                if (parentNode !== addFileButtonParent) {
                    ...
                    addFileButtonParent.style.height="58px";
                }
            }
        };
