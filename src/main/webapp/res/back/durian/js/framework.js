...
//	});
	$(".dodo_show_thumbnail").live({
		mouseenter : function(){
				...
				mouseleave : function(){$("#dodo_show_thumbnail_div").empty().hide();}
	});
	$._exception_alert_info_=function(msg,callback){
		callback = callback ||function(index){try{layer.close(index);}catch(ex){layui.layer.close(index);}};
		try{
			layer.alert(msg, {closeBtn:0,icon: 5},callback);
		}catch(ex){
			try{
				layui.layer.alert(msg, {closeBtn:0,icon: 5},callback);
			}catch(exx){
				alert(msg);
				callback();
			}
